...
module.exports.run = function ({ event, args, api, client, utils }) {
    ...
    switch (args[0]) {
        ...
        case "load": {
            ...
            else return loadCommand({ moduleList, threadID, messageID, client });
        }
        case "unload": {
            if (moduleList.length == 0) return api.sendMessage("لا يمكن أن تكون أسماء الوحدات فارغة🤖!", threadID, messageID);
            else return unloadModule({ moduleList, threadID, messageID, client });
        }
        case "loadAll": {
